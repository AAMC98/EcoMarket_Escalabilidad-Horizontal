...
2) Ejecuta un consumer para una queue concreta (puedes usar `notificaciones_consumer.py` o crear un consumer simple con `pika`).
3) Ejecuta los scripts de ejemplo para ver diferencias en comportamiento.
...
- Ejecutar una simulación simple (script) que publique N eventos y mida latencias por enfoque (requiere ejecutar consumers y medir).  
- Preparar un README corto con comandos exactos para ejecutar las pruebas en Windows PowerShell.
